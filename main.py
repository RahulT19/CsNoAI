...
TRACKED_EQUITIES = ["EA", "TTWO", "SONY", "NTDOY", "NVDA"]
...
class CsNoAIApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        # Hardcoded ticker values here to prevent scraper import errors
        ttk.Combobox(controls, textvariable=self.ticker, values=TRACKED_EQUITIES, state="readonly", width=10).pack(side="left", padx=(6, 16))
    ...
    def run_analysis(self):
        try:
            ...
            self.current = (scraped, frame, forecast, signal)
    ...
    def _render(self, scraped, frame, forecast, signal):
        ...
        self.high_metrics.set(self._metrics_text("High", forecast.high_model, forecast.horizon))
        self.low_metrics.set(self._metrics_text("Low", forecast.low_model, forecast.horizon))
